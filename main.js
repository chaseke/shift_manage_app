...
function renderCalendar() {
    ...
    shiftData.forEach((data, index) => {
        ...
        const hasKoma = (totalKoma > 0);
        let bgColorClass = 'bg-empty';
        if (hasDetails)
            bgColorClass = 'bg-detail';
        else if (hasKoma)
            bgColorClass = 'bg-active';
        const dayNum = data.dateObj.getDate();
        cell.innerHTML = `
...
window.changeKoma = function (index, amount) {
    if (amount > 0) {
...
window.openModal = function (index) {
    ...
    const dayNum = shiftData[index].dateObj.getDate();
    ...
    document.getElementById('modal-adjust').value = shiftData[index].adjust.toString();
...
window.changeModalTutor = function (amount) {
...
window.changeModalAdjust = function (value) {
    if (currentEditIndex === -1)
        return;
...
// 高校生コマの割合変更 (総コマ数は維持)
window.changeModalHighSchool = function (amount) {
    if (currentEditIndex === -1)
        return;
    const data = shiftData[currentEditIndex];
    const totalKoma = data.jrHigh + data.highSchool;
    if (totalKoma === 0)
        return; // コマが0の場合は割合変更不可
...
function renderRegularSettings() {
    ...
    if (!container)
        return;
    container.innerHTML = '';
    const displayOrder = [1, 2, 3, 4, 5, 6, 0]; // 月〜日の順
...
window.changeReg = function (dayIndex, type, amount) {
    if (type === 'jrHigh') {
...
window.applyRegularShifts = function () {
...
window.showExpectedIncome = function () {
    let totalIncome = 0;
    let workDays = 0;
    shiftData.forEach(data => {
        let k = data.jrHigh + data.highSchool;
